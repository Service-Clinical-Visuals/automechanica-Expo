  ...
  return (
    <header
      className={`fixed left-0 right-0 z-[100] transition-all duration-300 w-full flex justify-center ${isHidden ? "-translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
          ...
          ? "top-0 bg-[var(--background)]/95 backdrop-blur-md shadow-lg py-1 md:py-1.5"
    ...
    >
      ...
      <div className={`custom-container flex items-center justify-between transition-all duration-300 ${isScrolled
        ...
        : "rounded-[1.5rem] border border-white/40 bg-[var(--background)]/80 backdrop-blur-sm px-6 lg:px-8 py-1.5 md:py-2"
        }`}>

        {/* Logo Area */}
        <div className="flex-shrink-0 flex items-center w-[45%] sm:w-[35%] md:w-[25%] lg:w-[20%] xl:w-[15%]">
          <Link href="#" className="w-full">
            <img src="/moto/autocam/logo.png" alt="Autocom Logo" className="w-[90%] md:w-[85%] lg:w-full h-auto object-contain" />
          </Link>
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-10">
          {navLinks.map((link, idx) => (
            ...
            >
              <Typography variant="span" color="none" className={`navbar transition-colors ${idx === 0 ? "text-[#00A3E0]" : "text-white group-hover:text-[#00A3E0]"}`}>
                {link.name}
              </Typography>
            </Link>
          ))}
