...
export default function Header() {
  ...
  return (
    ...
    >
      <div className={`mx-auto flex items-center justify-between bg-[#1b1b1b] transition-all duration-300 px-8 ${
        isScrolled 
          ? "w-full px-4 xl:px-14 py-3 border-b border-[#ffffff]/30 shadow-2xl" 
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`header-link transition-colors whitespace-nowrap tracking-wide oswald-font  ${
                link.active
                  ? "text-[#F8AC1A] underline underline-offset-2 font-semibold"
                  : "text-white hover:text-[#F8AC1A] font-normal hover:underline underline-offset-2 hover:font-semibold"
              }`}
            >
              {link.name}
            </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-8  py-3 header-link oswald-font border-b border-gray-800/50 ${
                  link.active
                    ? "text-[#F8AC1A] font-semibold underline underline-offset-2"
                    : "text-white hover:text-[#F8AC1A] font-medium hover:underline underline-offset-2 hover:font-semibold"
                }`}
              >
                {link.name}
              </Link>
