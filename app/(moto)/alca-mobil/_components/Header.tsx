...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`top-0 left-0 w-full z-50 transition-all duration-500 transform bg-white ${isSticky || mobileMenuOpen
          ? "fixed translate-y-0 opacity-100 shadow-md"
      ...
      >
        <div className="h-[70px] sm:h-[80px] md:h-[90px] flex items-center relative z-10">
          <div className="custom-container flex items-center justify-between gap-4 xl:gap-5 w-full">
                ...
                return (
                  <div key={link.name} className="relative flex items-center justify-center">
                    <a
                      ...
                      className={`relative z-10 cursor-pointer py-3 px-1 mx-2 flex flex-col items-center justify-center transition-colors ${isActive ? "text-primary" : "text-foreground hover:text-primary"
                        }`}
                    >
            ...
            {/* Right Action: Language & Search */}
            <div className="hidden xl:flex items-center shrink-0 gap-4">
              <div className="flex items-center gap-2 border border-gray-200 rounded-md px-3 py-2 cursor-pointer hover:border-primary transition-colors">
                <img src="https://flagcdn.com/w20/gb-eng.png" alt="English" className="w-5 h-auto" />
                <span className="navbar text-foreground">English</span>
              ...
              <div className="w-10 h-10 border border-gray-200 rounded-full flex items-center justify-center cursor-pointer hover:border-primary hover:text-primary transition-colors text-primary">
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden absolute top-full left-0 w-full bg-white shadow-2xl py-6 px-6 flex flex-col space-y-4 max-h-[calc(100vh-70px)] overflow-y-auto border-t border-gray-100">
            {navLinks.map((link) => {
              ...
              return (
                <div key={link.name} className="relative border-b border-gray-100 pb-2">
                  <a
                    href={`#${link.id}`}
                    className={`relative z-10 block navbar cursor-pointer py-2 ${isActive ? "text-primary" : "text-foreground hover:text-primary"
                      }`}
                    onClick={(e) => handleScrollTo(e, link.id)}
                  >
                    {link.name}
            ...
            <div className="pt-6 flex flex-col gap-4">
              <div className="flex items-center justify-between border border-gray-200 rounded-md px-3 py-2 cursor-pointer hover:border-primary transition-colors w-full">
