...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        <div className="h-[70px] sm:h-[80px] md:h-[90px] flex items-center relative z-10 bg-secondary">
          <div className="custom-container flex items-center justify-between gap-4 xl:gap-5 w-full">
            ...
            <nav className="hidden xl:flex items-center space-x-6 2xl:space-x-8 shrink-0">
              {navLinks.map((link) => {
                ...
                return (
                  <div key={link.name} className="relative flex items-center justify-center">
                    <a
                      ...
                      className={`relative z-10 cursor-pointer py-3 px-1 flex flex-col items-center justify-center transition-colors ${isActive ? "text-white" : "text-white/70 hover:text-white"
                        }`}
                    >
                      <span className={`navbar ${isActive ? 'font-bold' : 'font-medium'}`}>
                        {link.name}
                      </span>
                    </a>
                  </div>
            ...
            <div className="hidden xl:flex items-center shrink-0">
              <Button text="Request Quote" href="#contact" showIcon={true} />
            </div>
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden absolute top-full left-0 w-full bg-secondary shadow-2xl py-6 px-6 flex flex-col space-y-4 max-h-[calc(100vh-70px)] overflow-y-auto border-t border-white/10">
            {navLinks.map((link) => {
              ...
              return (
                <div key={link.name} className="relative border-b border-white/5">
                  <a
                    href={`#${link.id}`}
                    className={`relative z-10 block navbar cursor-pointer py-4 ${isActive ? "text-white font-bold" : "text-white/70 hover:bg-white/5"
                      }`}
                    onClick={(e) => handleScrollTo(e, link.id)}
                  >
                    {link.name}
            ...
            <div className="pt-6 flex flex-col items-center gap-4 w-full">
               <Button text="Request Quote" href="#contact" className="w-full" showIcon={true} />
            </div>
          </div>
        )}
      </header>
