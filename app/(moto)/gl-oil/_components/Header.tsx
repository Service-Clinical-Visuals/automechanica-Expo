    ...
    <>
      <header
        className={`top-0 left-0  w-full z-50 transition-all duration-500 transform ${isSticky || mobileMenuOpen
          ? "fixed translate-y-0 opacity-100 shadow-md bg-[#111111]"
      ...
      >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <div key={link.name} className="relative flex items-center justify-center">
                    ...
                    >
                      <Typography variant="span" color="white" className="navbar tracking-wide">
                        {link.name}
                      </Typography>
                      {isActive && (
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden absolute bg-[#111111] top-full left-0 w-full  shadow-2xl py-6 px-6 flex flex-col space-y-4 max-h-[calc(100vh-70px)] overflow-y-auto border-t border-white/10">
            {navLinks.map((link) => {
              const isActive = activeLink === link.id;
              return (
                <div key={link.name} className="relative border-b border-white/5">
                  <a
                    href={`#${link.id}`}
                    className={`relative z-10 block cursor-pointer py-4 ${isActive ? "text-secondary" : "text-white hover:bg-white/5"
                      }`}
                    onClick={(e) => handleScrollTo(e, link.id)}
                  >
